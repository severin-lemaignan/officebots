...
func create_file(name): 
	...
	file.store_line("user : %s"%name + " (time,x,z,rotation)")
	
	file.close()
	
func save_data(id, data): 
	var path_modified = path + "/%s"%id + ".csv"
...
func pre_save():
	for p in $Players.get_children(): 
		...
		var data = "%s"%time+ "," + "%s"%p.global_transform.origin[0]+ "," + "%s"%p.global_transform.origin[2] +"," + "%s"%p.rotation_degrees[1]
		save_data(ID,data)
